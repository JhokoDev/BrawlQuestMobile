...
import com.google.mediapipe.examples.poselandmarker.detectors.AbdominalDetector
import com.google.mediapipe.examples.poselandmarker.detectors.PushUpDetector
import com.google.mediapipe.examples.poselandmarker.detectors.SquatDetector
...
class PoseLandmarkerHelper(
    ...
    // Adicionado: Instância de PushUpDetector
    private val pushUpDetector = PushUpDetector()
    private val squatDetector = SquatDetector()
    private val abdominalDetector = AbdominalDetector()


    // Added: Fields to track exercise counts
    private var pushUpCount = 0
    private var squatCount = 0
    private var abdominalCount = 0
    init {
        setupPoseLandmarker()
    }
    ...
    private fun returnLivestreamResult(
        ...
        // Adicionado: Chamada para detectar exercicios
        if (GameStateManager.getMissao().equals("Flexao")) {
            pushUpDetector.detectPushUp(result)
        } else if (GameStateManager.getMissao().equals("Agachamento")) {
            squatDetector.detectSquat(result)
        } else if (GameStateManager.getMissao().equals("Abdominal")) {
            abdominalDetector.detectAbdominal(result)
        } else {
